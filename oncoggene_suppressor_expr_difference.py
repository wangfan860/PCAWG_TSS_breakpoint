...
if __name__ == '__main__':
    ...
    old_expr_onco_fc=disease_old_expr_onco[onco.gene_y.tolist()].sub(onco_median_list, axis='columns')
    for i in old_expr_onco_fc.columns:
        old_expr_onco_fc.loc[(old_expr_onco_fc[i]>0),i]=1
        old_expr_onco_fc.loc[(old_expr_onco_fc[i]<0),i]=-1
    ...
    onco_table.fillna(0).to_csv('/gpfs/data/lyang-lab/users/fan/expr_difference_by_cancer_type/onco_foldchange_{}.csv'.format(disease))
    ...
    new_expr_suppr_fc=disease_new_expr_suppr[suppr.gene_y.tolist()].sub(suppr_median_list, axis='columns')
    for i in new_expr_suppr_fc.columns:
        new_expr_suppr_fc.loc[(new_expr_suppr_fc[i]>0),i]=1
        new_expr_suppr_fc.loc[(new_expr_suppr_fc[i]<0),i]=-1
    ...
    suppr_table.fillna(0).to_csv('/gpfs/data/lyang-lab/users/fan/expr_difference_by_cancer_type/suppr_foldchange_{}.csv'.format(disease))
